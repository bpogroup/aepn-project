...
class AEPN_Env(Env):
    ...
    def __init__(self, aepn):
        ...
        # gym specific
        self.observation_space = spaces.Box(shape=(len(pn_observations),), low=0, high=np.iinfo(np.uint8).max,
                                            dtype=np.uint8)  # observations are the state representation that is shown to the solving algorithm
        self.action_space = spaces.Discrete(
            len(self.pn_actions))  # actions are all the available decisions (invalid actions are defined on a single observation through a masking mechanism)
