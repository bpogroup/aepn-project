...
class HeteroGraphDeletionEnv(gym.Env):
    def __init__(self, graph=setup_example_graph_env()):
        super(HeteroGraphDeletionEnv, self).__init__()

        # Define the action space as the nodes in the graph
        self.action_space = spaces.Discrete(len(graph.nodes))

        # Define the observation space
        # In general, the Graph space would not be usable to encode heterogeneous graphs. To do so, we consider the maximum
        # amount of features per node (padding on nodes features) + 1 for the node type
        self.observation_space = spaces.Dict(
            {
                'graph': spaces.Graph(node_space=spaces.Box(low=0, high=len(graph.nodes), shape=(2,)), edge_space=None), #node_space contains two values: the first identifies the node index (e.g. how many features the node expects) while the second is a unique token identifier (to retrieve the actual values of the nodes attributes in the graph)
                'nodes_features': spaces.Dict({'node_id': spaces.Discrete(len(graph.nodes)), 'node_features'})
            }
        )
    ...
    def _get_obs(self):
        ...
        #return obs
        return {'graph': spaces.GraphInstance(nodes=x, edges=None, edge_links=edge_index), 'features': spaces.Dict({})}
...
if __name__ == '__main__':
    ...
    original_graph.add_nodes_from([(0, {'id': 0, 'reward': 0}), (1, {'reward': 1}), (2, {'id': 1, 'reward': 0})])
    original_graph.add_edges_from([(0, 1), (1, 2)])

    # Create the Gym environment
    env = HeteroGraphDeletionEnv(original_graph)
    done = False
